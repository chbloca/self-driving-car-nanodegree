...
float motion_model(float pseudo_position, float movement, vector<float> priors,
  ...
  for(int j = 0; j < map_size; j++){
      ...
      float delta_ij =  pseudo_position - next_pseudo_position;
      float P_transition = Helpers::normpdf(delta_ij, movement, control_stdev);
      position_prob += P_transition * priors[j];
  }
